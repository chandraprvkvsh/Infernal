...
class TestCLI:
    ...
    def test_pull_with_url(self, mock_class, runner, mock_infernal):
        ...
        assert result.exit_code == 0
        mock_infernal.pull_from_url.assert_called_once()
    ...
    def test_pull_with_repo_id(self, mock_class, runner, mock_infernal):
        ...
        assert result.exit_code == 0
        mock_infernal.pull_model.assert_called_once_with('test/repo', 'model.gguf')
    ...
    def test_pull_no_arguments(self, mock_class, runner, mock_infernal):
        ...
        assert result.exit_code == 0
        assert 'Provide either --url or both --repo-id and --filename' in result.output
    ...
    def test_run_command_with_prompt(self, mock_class, runner, mock_infernal):
        ...
        assert result.exit_code == 0
        mock_infernal.run_inference.assert_called_once_with(
    ...
    def test_benchmark_command_no_prompt(self, mock_class, runner, mock_infernal):
        """Test benchmark command without prompt"""
        mock_class.return_value = mock_infernal
    ...
    @patch('infernal.parse_modelfile')
    @patch('infernal.InfernalLLM')
    def test_finetune_command(self, mock_class, mock_parse, runner, mock_infernal, tmp_path):
        ...
        modelfile.write_text("FROM test/model")
        
        with patch('infernal.AutoTokenizer'), \
             patch('infernal.AutoModelForCausalLM'), \
             patch('infernal.Trainer') as mock_trainer:
            ...
            # Should not fail even if training is mocked
